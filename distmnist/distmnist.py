...
ps_hosts = ["slave2:2222"]
worker_hosts = ["slave2:2223", "slave2:2224"]
...
def main(_):
    ...
    if FLAGS.job_name == "ps":
        ...
        with tf.device(tf.train.replica_device_setter(
            ...
            ops = []
            for i in range(len(grads_list[task_index])):
                saveop = grads_list[task_index][i][0].assign_add(grads_and_vars[i][0])
                ops.append(saveop)

            #gradients check ops
            testops = []
            for i in range(len(grads_list[task_index])):
                boolop = tf.reduce_all(tf.equal(grads_list[task_index][i][0], grads_and_vars[i][0]))
        ...
        with sv.prepare_or_wait_for_session(server.target) as sess:
            for i in range(training_epochs):
                ...
                for i in range(len(ops)):
                    sess.run(ops[i], feed_dict={x:batch[0], y_:batch[1], keep_prob:1})
